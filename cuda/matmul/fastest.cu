// nvcc -O3 --use_fast_math -Xcompiler -fopenmp ./fastest.cu -o matmul
...
__global__ void matmul_kernel(float *A, float *B, float *C, int M, int N,
  ...
  const int threadRow = threadIdx.x / TILENUM;
  const int threadCol = threadIdx.x % TILENUM;
  ...
  __shared__ float As[BK * BLOCKSIZE];
  __shared__ float Bs[BK * BLOCKSIZE];
