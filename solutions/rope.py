...
def rope_kernel(
    ...
    d_half = d // 2
    for i in range(bh):
        t_ptrs = t_start_ptr + i * BLOCK_SIZE + col_offsets
        o_ptrs = o_start_ptr + i * BLOCK_SIZE + col_offsets
        t = tl.load(t_ptrs)
...
if torch.allclose(triton_output, torch_output, atol=1e-2, rtol=0):
    ...
    print(triton_output[1, 0, 1, :277])
    print(torch_output[1, 0, 1, :277])
    print(torch.sum(torch.abs(triton_output - torch_output) > 1e-2))
